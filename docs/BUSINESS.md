...
### 2026-08-18 — GA4 tracking fix (from Cowork audit; implemented same day)
- **⚠️ CLEAN BASELINE STARTS 2026-08-18.** Internal Traffic filter activated (was stuck in Testing with NO IP rule defined — it had never matched anything). Rule "Sarah home office" = 174.162.35.31/32. Everything before this date includes Sarah/team self-visits; do not compare across it. If Sarah's home IP changes (residential IPs rotate), re-check the rule.
- Code (all live): `trackLead()` in lib/analytics.ts — every lead event (contact, cash offer, VIP, grant, inherited) + phone clicks now ALSO fire `generate_lead`, which populates GA4's Lead acquisition reports (they were structurally 0). PhoneClickTracker now covers ALL tel:/mailto: links incl. nav/footer (per-link handlers removed; capture-phase; link_text + page_path params). GrantWizard raw gtag call routed through helper. `getClientId()` captured on all 5 lead forms → stored in Sierra note as "[GA client ID: …]" for future qualify_lead/close_convert_lead via Measurement Protocol. `valuation_tool_viewed` intent event on the Sierra iframe.
- GA4 admin: starred `grant_lead_submitted` + `listing_search_submitted` as key events (grant_qualification_submitted left unstarred — mid-funnel, would double-count); `purchase` key event CANNOT be removed (GA4 built-in default, grayed out — harmless); `sierrasellersites.com` added to unwanted referrals (vercel.app was already there).
- Lead values: generate_lead fires without a value for now (counts work; value-weighted reporting off). To add values later, pass `value:` per lead type in lib/analytics.ts wrappers.
- Still open from the audit: email Sierra about a postMessage/redirect on valuation-tool submit (draft below in backlog); until then /home-valuation only tracks intent, not conversions.
...
**Press release LIVE (published 2026-08-19):** https://www.einpresswire.com/article/935180245/rochester-s-the-patrick-group-recognized-by-hour-detroit-as-a-real-estate-all-star — picked up by AP News, Google News, WLNS CBS 6, WOOD NBC 8, Bloomberg Terminal, Michigan Newswire (distribution report: einpresswire.com/report/ZdrT_DfwJPhNWc_P). Now featured on the site's /news page (data in `data/news.ts`; add future mentions there, one object per entry, nothing else to edit). Homepage has an "As Seen In" strip. TODO Sarah: submit thepatrickgrp.com/news in Search Console URL inspection; add /news to email signatures; tag Hour Detroit + Real Producers when sharing.

**Press release submitted to EIN Presswire 2026-08-18** ($149 basic, US + Real Estate category): Hour Detroit All-Stars + Real Producers cover angle, 3 anchor links (thepatrickgrp.com, /market-tracker, /grants), GBP map embed, Sarah+Brad photo. Full text: `~/Desktop/patrick-group/content/press-release-2026.md`. Awaiting editorial approval, then distribution to 550+ sites; pickup report to follow. When live: add link to site press mentions + this log.
...
- Email Sierra support: ask whether the seller-site valuation tool can postMessage on submit or redirect to a thank-you URL on thepatrickgrp.com (gives us a real conversion event for /home-valuation). Draft: "Hi, we embed our valuation tool (9b0ccdbd79e34649b94c438625c4294f.sierrasellersites.com) in an iframe on thepatrickgrp.com/home-valuation. Is there a way to (a) have the widget post a message to the parent window when a visitor submits, or (b) configure a redirect to a URL on our domain after submission? We want to record the conversion in our analytics. Thanks, Sarah Patrick"
