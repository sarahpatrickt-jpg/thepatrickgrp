...
function Stars() {
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((i) => (
        <svg key={i} className="w-4 h-4 text-[var(--red)] fill-current" viewBox="0 0 20 20">
    ...
    <>
      ...
      >
        <div className="max-w-3xl mx-auto text-center">
          ...
          <div className="inline-flex flex-col items-center gap-2 bg-[var(--paper)]/10 px-8 py-5">
            <p className="font-display text-5xl text-white">5.0</p>
            <div className="flex gap-1">
              {[1,2,3,4,5].map((i) => (
                <svg key={i} className="w-6 h-6 text-[var(--red)] fill-current" viewBox="0 0 20 20">
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
              ))}
            </div>
            ...
            <a
              ...
              className="text-[var(--red)] text-xs font-semibold hover:underline"
            >
        ...
        <div className="max-w-5xl mx-auto">
          <div className="columns-1 sm:columns-2 lg:columns-3 gap-6 space-y-6">
            {reviews.map((r) => (
              <div
                key={r.name}
                className="break-inside-avoid bg-[var(--paper)] border border-[var(--line)] p-6 shadow-sm"
              >
                {/* Quote mark */}
                <div className="text-[var(--red)] text-4xl font-display leading-none mb-3 select-none">&ldquo;</div>
                <p className="text-[var(--ink-2)] text-sm leading-relaxed mb-5">{r.text}</p>
                <div className="border-t border-[var(--line)] pt-4 flex items-center justify-between">
                  <p className="font-semibold text-[var(--ink)] text-sm">{r.name}</p>
                  <Stars />
                </div>
              </div>
            ))}
          ...
          <div className="mt-12 text-center">
            <p className="text-[var(--ink-3)] text-sm mb-4">Reviews sourced from Google. See our full listing on Google.</p>
            <a
              href="https://www.google.com/search?q=The+Patrick+Group+Oak+and+Stone+Real+Estate"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 border border-[var(--line)] px-6 py-3 text-sm font-semibold text-[var(--ink)] hover:border-[#c70000] hover:text-[var(--red)] transition-colors"
            >
      ...
      <section className="py-16 px-4 sm:px-6 bg-[var(--ink)] text-white text-center">
        <p className="text-[var(--red)] uppercase tracking-[0.22em] text-[11px] font-medium font-mono mb-3">Ready to get started?</p>
